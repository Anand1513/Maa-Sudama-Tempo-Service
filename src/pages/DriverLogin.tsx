...
import { Truck, ArrowLeft, Mail, Smartphone, UserPlus, CheckCircle2, Circle } from "lucide-react";
...
const DriverLogin = () => {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      // Demo validation
      if (identifier === "driver@msts.com" && otp === "123456") {
        toast.success("Successfully logged in!");
        navigate("/driver-dashboard");
        return;
      }
      ...
      <div className="w-full max-w-md bg-card p-8 sm:p-10 rounded-3xl border border-border shadow-2xl">
        <div className="flex flex-col items-center mb-10 text-center">
          <div className="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center text-primary-foreground mb-5 shadow-lg shadow-primary/20">
            {step === 1 && !isRegistering && <Truck className="w-8 h-8" />}
            {step === 1 && isRegistering && <UserPlus className="w-8 h-8" />}
            {step === 2 && (isEmail ? <Mail className="w-8 h-8" /> : <Smartphone className="w-8 h-8" />)}
            {step === 3 && <Truck className="w-8 h-8 text-yellow-300" />}
          </div>
          
          <h2 className="text-3xl font-display font-medium text-foreground tracking-tight">
            {step === 1 ? (isRegistering ? "Register Driver" : "Driver Portal") :
             step === 2 ? "OTP Verification" : "Registration Pending"}
          </h2>
          
          <p className="text-muted-foreground mt-2">
            {step === 1 && !isRegistering && "Sign in to view your assigned trips and routes"}
            {step === 1 && isRegistering && "Register to join the MSTS fleet"}
          ...
          <form onSubmit={handleAuth} className="space-y-5 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="py-6 rounded-xl"
                placeholder="driver@msts.com or 9876543210"
              />
            </div>
            
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-sm font-semibold text-foreground">{isRegistering ? "Secure Password" : "Secure PIN / Password"}</label>
                {!isRegistering && <a href="#" className="text-sm text-primary hover:underline">Forgot PIN?</a>}
              </div>
              <Input
                type="password"
                required
            ...
            <Button type="submit" disabled={loading} className="w-full font-bold py-6 mt-4 text-base rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all">
              {loading ? "Processing..." : (isRegistering ? "Submit Application" : "Access Dashboard")}
            </Button>
            ...
            <div className="text-center mt-6">
              ...
              >
                {isRegistering ? "Already have an account? Sign In" : "New Driver? Apply Here"}
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="animate-in zoom-in-95 text-center p-6 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl">
            ...
            <p className="text-muted-foreground mb-4">
              Thank you for applying to drive with MSTS. Our administrative team must review your license and approve your account.
            </p>
            <p className="text-sm font-semibold text-foreground mb-6">
              You will receive a message on your registered {isEmail ? 'email' : 'mobile number'} within 24 hours once approved.
            </p>
